...
panmesh is accumulating architecturally significant decisions — e.g. which crate to use for XML parsing ([#17](https://github.com/tkoyama010/panmesh/issues/17)), how to handle GMSH files ([#19](https://github.com/tkoyama010/panmesh/issues/19)), and which crate to use for HDF5 heavy data in XDMF ([#18](https://github.com/tkoyama010/panmesh/issues/18)). Today these decisions live only in issue threads and PR descriptions, which makes the rationale hard to find later and easy to lose. We want to record architectural decisions made in this project — whether they concern the architecture, the code, or other fields — in a consistent, version-controllable way. Which format and structure should these records follow?
...
* **Low overhead**: panmesh is a small project; the format must be lean enough that writing a record does not feel like a burden.
...
3. Subsequent architecturally significant proposals (e.g. the crate choices tracked in [#17](https://github.com/tkoyama010/panmesh/issues/17), [#18](https://github.com/tkoyama010/panmesh/issues/18), [#19](https://github.com/tkoyama010/panmesh/issues/19)) are accompanied by a MADR record linked from the issue.
4. A CI step (markdownlint with MADR's `.markdownlint.yml` config) lints records on PRs touching `docs/decisions/`.
5. The workflow is briefly documented in the README so contributors know when and how to open an ADR.
...
* Good, because the project is actively maintained: 4.0.0 released 2024-09-17, with examples, a changelog, and tooling (markdownlint config, init scripts).
